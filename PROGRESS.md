...
## 第三轮：框选二次编辑 + 临时文件清理 + 开源发布（2026-08-31）✅

1. **框选后可二次编辑**：截图框选改为两阶段状态机——画框松手后进入编辑态（不立即消失）：8 向手柄调整大小（固定边锚定不漂移）、框内拖动整体移动、点框外重新画、Enter 或「翻译」按钮确认、Esc/✕/右键取消。实弹证据：画框 600×220 → 拖 SE 手柄调整为 450×130 → 框内拖动移位 → Enter → OCR 识别 124 字符 → 翻译弹窗（laguna · 4458ms）
2. **临时文件清理**：OCR 临时目录改为「启动时 + 每次识别完成后」自动整目录清理；deploy.ps1 清理图标补丁临时文件；开发期遗留的 TEMP/测试目录全部删除（TEMP 中 huayi-* 清零）
3. **开源发布**：提交至 https://github.com/shi-tou1234/fanyi （main 分支，含源码/图标/测试/文档/MIT）；Mimosa 深度安全扫描 0 findings（258 依赖全查，seal efee439c…）后才推送

第三轮执行中发现并修掉的坑：
- **安全扫描器目录锁**：ZCode 内置 Mimosa 扫描器会对项目内被扫描目录长持句柄（asar/目录删改全拒），不可杀 → 部署目标移出项目（%LOCALAPPDATA%\划译\app-<时间戳>，deploy 每次清旧建新）；electron-builder 输出同样移出项目后再把 exe 复制回 release-builder\
- rcedit 中文路径挂 → ASCII 临时目录中转
- 框选 resize 的固定边必须锚定按下时矩形（否则拖动中边缘漂移）
- 纯上下/左右手柄 resize 只更新受控轴

### 部署位置说明
- 便携目录版部署在 `%LOCALAPPDATA%\划译\app-<时间戳>\`，桌面快捷方式自动指向最新版
- 项目内 release\划译 与 release\app 目录曾被扫描器锁死残留（仅应用自带文件，重启电脑后可删）
